...
MyPrintInteractive <- function(p, plot_wo_widget, hover_css = "") {
  # github_markup generation does not like javascript generated by ggiraph, 
  # therefore having a switch to use a widget when needed
  if (plot_wo_widget) {
    if (hover_css == "")
    ...
    if (hover_css == "")
      ...
      widgetframe::frameWidget(ggiraph(code = {print(p)}, hover_css = hover_css))
  }   
}
...
CreateEuroMap <- function(dsin, ind_name, txt_title, txt_subtitle, txt_caption) {
  ...
  idx <- which(!(dsin$cntry_name %in% world$admin))
  if (length(idx) > 0) stop(paste("Missing countries!!", paste(ds_participations$cntry_name[idx], collapse = ",")))

  # external to internal name of the indicator  
  colnames(dsin)[colnames(dsin)==ind_name] <- "myplotind"
  
  # if we have factor to show, colour scale to discrete
  use_discrete <- (class(dsin$myplotind) == "factor")
  
  # gather geography beyond Europe, limiting what to show by coordinates
  europe <- world %>% 
    dplyr::filter(region_un %in% c('Europe','Africa','Asia')) %>%
    dplyr::left_join(dsin %>% 
                       dplyr::mutate(admin = as.character(cntry_name)) %>%
                       dplyr::select(admin, myplotind),
                     by = 'admin') %>%
    st_transform(crs="+proj=laea +lon_0=18.984375")
  
  # prepare map data
  #europe.centers <- st_centroid(europe)     # not used for now
  europe.spdf <- methods::as(europe, 'Spatial')
  europe.spdf@data$id <- row.names(europe.spdf@data)
  # supressing a warning and message which do not seem to have an impact 
  europe.tidy <- suppressWarnings( # warning "binding character and factor vector, coercing into character vector"
    suppressMessages(              # message "Regions defined for each Polygons"
      broom::tidy(europe.spdf)
    )
  ) 
  europe.tidy <- europe.tidy %>% 
    dplyr::left_join(europe.spdf@data, by='id')

  # the plot  
  g <- ggplot(europe.tidy) +
    geom_polygon_interactive(
      color='gray',
      aes(long, lat, group=group, fill=myplotind,
          tooltip=paste0(admin,
                         ifelse(is.na(myplotind),
                                '',                             # no info, no show
                                paste0('\n',                    # has info, new line
                                       round(                   # meaningful digits
                                         as.numeric(myplotind), # handle number as factor (country groups)
                                         2)))))) +
  hrbrthemes::theme_ipsum() +
    scale_fill_viridis(discrete = use_discrete, 
                       na.value = '#EEEEEE') +
    # units here are not exactly traditional lat/long
    xlim(c(-2.6e6, 1.8e6)) + ylim(c(3.1e6, 7.5e6)) +
...
GetObservationsSet <- function(ds, ds_obs, the_indicator, indicator_table, cntry_list){
  ...
  eu_map <- CreateEuroMap(dsin = ds_averages %>%
                            filter(ess_year=="Mean", cntry_name != "Mean"), 
                          ind_name = "var_ave",
